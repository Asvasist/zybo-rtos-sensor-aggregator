...
gpio_drv_status_t gpio_drv_init(void)
{
    ...
    for (btn = 0U; btn < (uint32_t)GPIO_BTN_COUNT; btn++)
    {
         ...
         * reset doesn't show up as a fresh press at boot. Right after the
         * pull-up change the pad may not have settled yet; if the seed is
         * wrong the debouncer corrects it within three scans, without
         * reporting a press.
         */
